...
//            ->withQueryString();
    ...
    public function show2(Request $request, Product $products)
    {
        if (!empty($request->get('keyword'))){
            $products = $products->where('title', 'like', '%'.$request->get('keyword').'%');
        }
        $products = $products::with('brand')->paginate(4);
    ...
    public function update(UpdateProductRequest $request, Product $product)
    {
        ...
        if(!Storage::exists('public/images/products/'.$image_name)){
            Storage::putFileAs('public/images/products/', $request->file('image'), $image_name);
        }
        ...
        //Quay về danh sách
        return Redirect::route('admin.product')->with('success', 'Update a product successfully!');
    }
